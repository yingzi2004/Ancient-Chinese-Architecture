...
namespace InventorySystem
{
    ...
    public class InventoryController : MonoBehaviour
    {
        ...
        private void ToggleOnKeyInput()
        {
            ...
            if (!TryGetPressedToggleKey(out string input))
            {
                return;
            }
        ...
        /// <summary>
        /// Tries to resolve a single-character key press for inventory toggle.
        /// Uses Input.inputString first, then falls back to KeyCode checks so IME layouts still work.
        /// </summary>
        private bool TryGetPressedToggleKey(out string input)
        {
            ...
            for (int i = 0; i < 26; i++)
            {
                KeyCode letterKey = (KeyCode)((int)KeyCode.A + i);
                if (Input.GetKeyDown(letterKey))
                {
                    input = ((char)('a' + i)).ToString();
                    return true;
                }
            }
            ...
            for (int i = 0; i < 10; i++)
            {
                KeyCode digitKey = (KeyCode)((int)KeyCode.Alpha0 + i);
                if (Input.GetKeyDown(digitKey))
                {
                    input = i.ToString();
                    return true;
                }
